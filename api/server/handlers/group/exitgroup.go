...
func ExitGroup(w http.ResponseWriter, r *http.Request) {
	...
	err = database.DB(DBNAME).C("Users").Find(bson.M{"email": email}).One(&user)
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
	...
	err = database.DB(DBNAME).C("Users").Update(bson.M{"email": email}, bson.M{"$pull": bson.M{"groups": muxvar}})
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
	...
	err = database.DB(DBNAME).C("Groups").Update(bson.M{"_id_": muxvar}, bson.M{"$inc": bson.M{"usercount": -1}, "$pull": bson.M{"members": user}})
	if err != nil {
		w.WriteHeader(http.StatusInternalServerError)
